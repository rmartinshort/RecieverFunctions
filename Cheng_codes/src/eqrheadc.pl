...
# perl scripts for add head to eqr files. 
 ...
 $DIR= "/home/weizg/A";
# $DIR= "/home/weizg/Documents/data--9.22/hbb1_130-badch";
my($onesta)=@ARGV;
...
# if( -d $dir && $dir =~ /029/){
 if( -d $dir ){
  ...
  for($st = 0; $st < @STATION; $st++ ){
    ...
    $stla= `saclst stla -h< $SACFILE `; chomp($temp);# $temp = `saclst stla -h< $SACFILE `; chomp($temp);
#    ($temp1,$stla) = split(' ',$temp); chomp($stla);
    $stlo = `saclst stlo -h< $SACFILE `; chomp($temp);
#    ($temp1,$stlo) = split(' ',$temp); chomp($stlo);
    $stel = `saclst stel -h< $SACFILE `; chomp($temp);
 #   ($temp1,$stel) = split(' ',$temp); chomp($stel);
    $stdp = `saclst stdp -h< $SACFILE `; chomp($temp);
  #  ($temp1,$stdp) = split(' ',$temp); chomp($stdp);
    $evla = `saclst evla -h< $SACFILE `; chomp($temp);
  #  ($temp1,$evla) = split(' ',$temp); chomp($evla);
    $evlo = `saclst evlo -h<$SACFILE `; chomp($temp);
#    ($temp1,$evlo) = split(' ',$temp); chomp($evlo);
    $evdp = `saclst evdp -h< $SACFILE `; chomp($temp);
 #   ($temp1,$evdp) = split(' ',$temp); chomp($evdp);
    $kstnm= `saclst kstnm -h< $SACFILE `; chomp($kstnm);
#    ($temp1,$kstnm) = split(' ',$temp); chomp($kstnm);
    $gcarc= `saclst gcarc -h< $SACFILE `; chomp($temp);
 ...
 #     print SAC "wh over\n";  add by myself 
    ...
    print STDOUT "The head of $eqrfile   was added!\n";
  }
 chdir("..");
...
sub ttimes{
  my ($depth,$gcarc,$num)=@_;
   #$tmp_file = "/home/weizg/tools/abc";
  $tmp_file = "/home/weizg/ttimes/abc";
  my @ll;
  my $l;
  my $t;
  my @c;
   open(TT,"|ttimes > $tmp_file") || die "Cannot start ttimes!\n";
  # open(TT,"|system("/home/weizg/tools/ttimes/ttimes") > $tmp_file") || die "Cannot start ttimes!\n";   2008/11/13 
  print TT "all\n\n$depth\n$gcarc\n-1\n-1\n";  close(TT);
...
#SAC> SAC> SAC> SAC> SAC> SAC> SAC> SAC> SAC> SAC> SAC> SAC> SAC> SAC> SAC> SAC>  ERROR interpreting command:  ch kstnm 209WG.Z � 
# ILLEGAL OPTION:                               ^
#SAC> SAC> SAC> SAC> The head of 209_03_2442316.eqr   was added!    so I change the different saclst
