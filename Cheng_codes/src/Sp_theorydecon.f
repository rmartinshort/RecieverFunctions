...
c  *************************************************************************
c
      dimension data(10000,3),temp(5000),caz(3)
      complex d(5000,3)
      dimension ef(5000),eb(5000),sz(5000),sr(5000),st(5000),hz(5000)
      dimension hr(5000),ht(5000),a1(5000),a2(5000)
      character eqfile*80,comp(3,2)*2,outc(3)*4
      character*32 evnam,outfil
      logical yes,yesno,rel,rmean,filter,rdatcut
      integer blank,ounit
      intrinsic aimag
c
c
...
c
    ...
    2 call asktxt('specify event name: ',evnam)
      ...
      sptime=ask('phase shift time: ')
      rdatcut=yesno('abstract the wavelet from the r or t data?' )
...
c      do 8 i=1,3
c      delay=0.
c      if(i.ne.1) delay=sptime
c      call dfftr(data(1,i),nft,'forward',dt)
c      do 9 j=1,nfpts
c      freq=float(j-1)*delf
c      w=2.*pi*freq
c      cc1=data(2*(j-1)+1,i)
c      cc2=data(2*j,i)
c      d(j,i)=cmplx(cc1,cc2)
c      d(j,i)=d(j,i)*exp(cmplx(0.,w*delay))
c    9 continue
c      call dfftr(d(1,i),nft,'inverse',delf)
c      do 111 j=1,nfpts
c      data(2*(j-1)+1,i)=real(d(j,i))
c      data(2*j,i)=aimag(d(j,i))
c 111  continue
c      outfil(1:iblank1+3)=evnam(1:iblank1)//'_'//comp(i,2)
...
c
 ...
 1101   continue
        ...
        if(rdatcut) then
        twindow=ask('wavelet duration: ')
        kk2=(sptime+twindow)/dt+1
        else
        kk2=npts
        endif
        kk1=sptime/dt+1
        call getdata(data(1,1),kk1,kk2,npts)
      do 8 i=1,3
      j=i
      if(i.eq.1) j=ks
      if(i.eq.ks) j=1 
      outfil(1:iblank1+3)=evnam(1:iblank1)//'_'//comp(j,2)
...
c
       do 11 i=2,3
        j=i
        if(i.eq.ks) j=1
        outfil(1:iblank1+4)=evnam(1:iblank1)//outc(j)
        call wsac1(outfil,data(1,i),npts,beg,dt,nerr)
   11  continue
