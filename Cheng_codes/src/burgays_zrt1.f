...
c    *** modifid by Ling Chen for ZRT components 06/19/04
c
c  *************************************************************************
c
...
c      wrot=yesno('write the raw rotated data (y or n)? ')
      ...
      open(10,file=
     & '/work/common/home/chl/RF/program/decon/xww/burgays1_test.in',
     & status='old')
      read(10,'(a)')charatemp
      read(10,*)bptime,eptime
      read(10,'(a)')charatemp
      read(10,*)agauss,c
      close(10)
...
c
*ling       if(rel) call rotate(data,LENGTH,3,baz,caz,npts)
