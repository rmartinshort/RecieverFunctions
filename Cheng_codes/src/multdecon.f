...
c
      ...
      dimension aa(10000,4000), bb(80000)
      parameter (ndat = 80000)
      real ss(4000),wk(8000),sol(8000)
      complex data(4500,3)
...
c
      ...
      do 5555 i = 1, npts
         ...
         rzr(i) = rzr(i) + tmp2(i)
         rzt(i) = rzt(i) + tmp3(i)
         bb(i+npts*(ifile-1)) = tmp2(i)
c        aa(ifile*i,i) = tmp1(i)
      do 7777 mmm = (ifile-1)*npts+1, ifile*npts
        do 8888 nnn = i, npts
         aa(mmm,nnn) = tmp1(nnn-i+1)
8888    continue
        do 9999 nnn = 1, i
         aa(mmm,nnn) = tmp1(npts-i+1)
9999    continue
7777  continue 
5555  continue
6666  continue
...
70    continue
 ...
 80   continue
      ...
      ip  = npts
      write(*,*) 'here'
      call lsvdf( aa, ndat, npb, ip, bb, ndat, 1, ss, wk, ier)
      write(*,*) 'svd'
      call csoln( aa, ndat, npb, ip, bb,  ss, sol ) 
      write(*,*) 'end'
c      do 111 i=1,3
        outfil(1:iblank+4)=eqfile(1:iblank)//outc(i)
c        call wsac1(outfil,data(1,i),npts,beg,dt,nerr)
        call wsac1(outfil,sol,npts,beg,dt,nerr)
c 111    continue
