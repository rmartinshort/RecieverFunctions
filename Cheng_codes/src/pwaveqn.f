...
c      call asktxt('specify station name: ',stanam)
      call asktxt('specify eqfile : ',eqfile)
      rel=yesno('Geographic(zne) data (y or n)? ')
      b1=ask('p wave beginning time before A (in Sec positive): ')
      e1=ask('p wave ending after A (in Sec): ')
      rmean=yesno('remove mean value (y or n)?')
c      iblank1=blank(evnam)
c      iblank2=blank(stanam)
c      iblank=22+iblank1+iblank2
...
c         eqfile(1:iblank + 3)=eqfile(1:22)//evnam(1:iblank1)
...
c
 ...
 111  continue
      ...
      do 11 i=2,3
           outfil(1:iblank + 4)=eqfile(1:iblank)//outc(i-1)
