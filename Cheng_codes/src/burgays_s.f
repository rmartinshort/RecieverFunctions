...
C    modified by Ling Chen for S receiver function calculation
C    on 01/27/06
c
c  *************************************************************************
c
      parameter(LENGTH=20000)
      parameter(LENG1=20000)
      parameter(LENG2=20000)
      dimension data(LENGTH,3),temp(LENGTH),caz(3)
      complex d(LENG1,3)
...
c      dimension hr(4000),ht(4000),a1(4000),a2(4000)
      character eqfile*32,comp(3,3)*4,outc(3)*4               ! 12/07/06
...
c
      ...
      data
     *     comp/'.bhz','.bhn','.bhe','.shz','.shn','.she',    ! 12/07/06
     *          '.z','.r','.t'/
     *    ,outc/'.eqr','.eqz','.eqt'/
      inunit=5
      ounit=6
...
C Ling:
      ks=2
c      ks=ask('which chanel as decon: 2 (r) or 3 (t) ') 

      open(10,file=
     & '/work/common/home/chl/RF/program/decon/xww/burgays_s.in',
     & status='old')
      read(10,'(a)')charatemp
      read(10,*)bstime,estime
      read(10,'(a)')charatemp
      read(10,*)swt,swt0
      read(10,'(a)')charatemp
      read(10,*)agauss,c
      read(10,'(a)')charatemp                                 ! 12/07/06
      read(10,*)ifband
      close(10)
...
C Ling:
*      swt=20.0      
c      swt=ask('S wavelet duration (in Sec): ')
      yes=.true.
      blen=5
      rmean=.true.
      filter=.true.
...
c      eqfile(1:16)='/mnt/tibet/tele/'
      outfil(1:32)='                                '
      isyntp=3                                                ! 12/07/06
      if(rel) isyntp=ifband+1                                 ! 12/07/06
      iblank = blank(eqfile)
...
c
*       write(*,*)'beg=',beg,' a0=',a0,' bstime=',bstime
5      k11=1
       if(bstime.gt.0.) k11=(a0-bstime-beg)/dt+1
       k12=(estime+a0-beg)/dt+1
       beg=beg-a0
       if(bstime.gt.0.) beg=-bstime
       do 3 i=1,3
       call getdata(data(1,i),k11,k12,npts)
  3    continue
       npts=k12-k11+1
*       write(*,*)'beg=',beg,' k11=',k11,' k12=',k12

C Ling:
*       write(*,*)'beg=',beg,' swt0=',swt0,' swt=',swt
       k11=(-beg-swt0)/dt+1
       k12=(-beg+swt-swt0)/dt+1
*       call getdata(data(1,2),k11,k12,npts)
*       call getdata(data(1,3),k11,k12,npts)
       do i=1,npts
         if(i.lt.k11.or.i.gt.k12) data(i,ks)=0.
       enddo
*       write(*,*)' k11=',k11,' k12=',k12
...
c 4    continue
 ...
 1101   continue
        ...
        do 1102 j=1,3
        tempmy=data(i,j)
        data(i,j)=data(npts-i+1,j)
        data(npts-i+1,j)=tempmy
 1102   continue
        beg=-estime
C Ling end
